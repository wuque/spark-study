...
/**
  ...
  */
object WindowWordCountStreaming {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf()
        ...
        .setAppName("WindowWordCountStreaming")
    ...
    val checkpointDirectory = "/datas/sparkstreaming/stateKafka/00000"
    ...
    context.awaitTerminationOrTimeout(2 * 5 * 1000)
  }
  ...
  def processStreamData(ssc: StreamingContext): Unit = {
    ...
    /**
      * 窗口统计：每次统计最近6个batch的数据，每隔3batch时间执行一次
      */
    val wordCountsDStream: DStream[(String, Int)] = pairDStream.reduceByKeyAndWindow(
      (v1: Int, v2: Int) => v1 + v2, // reduceFunc: (V, V) => V
      Seconds(2 * 6), // windowDuration: Duration
      Seconds(2 * 3) // slideDuration: Duration
    )
    /**
